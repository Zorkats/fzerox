...
#ifdef PORT
/* Node-panel labels served from the loaded 64DD disk. Retail has no symbols for these
 * (pooled .rodata initializers), so the port keeps its own storage
 * (port/gen/EkTranslatedStrings.c), filled by gdx_ek_strings_apply() at disk-load time.
 * Declared locally: the generated header lives outside this TU's include path. Empty means
 * no translated disk is loaded and the Japanese below draws unchanged -- no build flag,
 * just whichever disk is inserted. */
...
/* Non-zero when the loaded 64DD image is the fan-translated release
   (port/gdx_ek_disk_overrides.c). The two places in this file where the translation changed
   GEOMETRY rather than text must test this and not a bound label string: the retail-JP disk
   has to keep the retail layout. */
extern int gdx_ek_disk_is_translated(void);

/* Retail draws each label at `left` and a right-aligned, space-padded numeric field at the
 * SAME `left` (func_xk2_800EDE68 pads, func_xk2_800EDAD0 skips a space, 19F3F0.c), so the
 * label sits inside the field's leading padding and English labels fit at retail coordinates.
 *
 * The fan translation is not a data-only patch: it rebuilt these overlays (physical 0x0C39100
 * = VRAM 0x800D6D90). The port compiles this file and substitutes only the disk's DATA, so
 * wherever the disk's geometry differs it must be followed explicitly. Exactly three places
 * differ: the node-panel header, the track-shape name, and the Create Machine weight caption
 * (machine_create_draw.c). Everything else -- Bank, Width, L/R, X/Y/Z, the "M" row and the
 * five count rows -- is retail-to-the-pixel in the disk's recompiled code with only the
 * string pointer changed. */

/* Pixel width of an EK setup-font string, bounded to `max` bytes: that font is a flat 8 px
   per byte (A6340.c:125). The bound exists because D_xk2_800F7090 is [][8] and the English
   disk fills two of its rows to all eight bytes with no terminator. */
...
/* NUL-terminated copy of at most `max` bytes so a row of D_xk2_800F7090 can be handed to
   _Printf as %s without running into the next row. `out` needs max+1 bytes. */
...
#ifdef PORT
    // ReduceEditorFlashing: steady marker highlights and a half-rate size pulse instead of the
    // ~20 Hz editor strobe; default 0 keeps everything bit-identical.
    //
    // The two marker indicators share the stock D_800DCCFC parity but need OPPOSITE frozen
    // polarities to stay visible (selected markers light green on the ON phase, the cursor node
    // shows white on OFF), so a single frozen phase cannot serve both. gGameFrameCount advances
    // once per rendered frame, so /2 is exactly half the native pulse rate.
...
#ifdef PORT
    // ReduceEditorFlashing, inverse phase of the skip in func_xk2_800E73DC (see the reasoning
    // there); this path skips the selected node's line on D_800DCD04 == 0.
    extern int CVarGetInteger(const char* name, int defaultValue);
    s32 gdxReduceFlash = CVarGetInteger("gEnhancements.Gameplay.ReduceEditorFlashing", 0);
...
void func_xk2_800E2238(Gfx** gfxP) {
...
#ifdef PORT
    /* Node-panel header. Retail's digits-then-word order is correct Japanese
       (aCourseEditNumberTex is a counter suffix) and backwards in English, so the translation
       swapped caption and number. Coordinates are the disk's own (VRAM 0x800E2708-0x800E27D4);
       the tens digit is TEN pixels wide, not twelve, because the pair must end at left+53 to
       stay off the frame. Gated on the disk, not a bound label: this is geometry, and the
       retail-JP disk keeps the Japanese order. */
    ...
    top++;
    {
...
#ifdef PORT
            {
                /* Track-shape name. D_xk2_800F7090 is [][8] with no terminator, and the
                   English disk fills rows 3/4 to all eight bytes, so the row read as a
                   _Printf format string runs into the next one. Mirror the disk's own code
                   (VRAM 0x800E3310-0x800E3348): measure bounded to 8 bytes, copy, centre at
                   left + 24 - width/2. The copy is NUL-terminated because an unterminated
                   format string is UB under the port's _Printf, not the harmless overread it
                   is on hardware. JP rows are 5-6 bytes, so that path is byte-identical. */
                u8 shapeName[9];
        ...
        D_xk1_80032AD0 = 0;
#ifdef PORT
        /* Bank row: retail geometry in both languages (provenance note at the top of the
           file). Widening the field to eight cells is what broke this row before; the word
           never needed shortening. */
        ...
        top += 3;
#ifdef PORT
        /* Width row: retail geometry in both languages, as the bank row. The jammed
           "Width52" with no gap is the disk's own layout, not a port defect. D_xk1_80032AD0
           stays 1 as retail sets it; it only matters for bytes >= 0x80. */
...
void func_xk2_800E38A8(Gfx** gfxP) {
    ...
    /* Course-length row: retail coordinates in both languages, as the bank row. */
...
Gfx* func_xk2_800E49FC(Gfx* gfx) {
    ...
    D_xk1_80032AD0 = 0;
    /* Count rows in the course-info box (this one and the four below) keep retail's cluster in
       both languages; the translated disk changes none of it. Shifting the cluster 8 px right
       is what pushed the dash denominator outside the box before, not word length. */
...
#ifdef PORT
    // ReduceEditorFlashing: the stock D_800DCD04 phase skip draws the selected node's line only
    // 1 game frame in 3 -- a soft pulse on console (3 scanout buffers + phosphor) but a hard
    // ~6.6 Hz blink on the port's digital present, and the marker-blink reducer (D_800DCCFC)
    // does not cover it. Off = stock skip, byte-identical.
    extern int CVarGetInteger(const char* name, int defaultValue);
    s32 gdxReduceFlash = CVarGetInteger("gEnhancements.Gameplay.ReduceEditorFlashing", 0);
#define GDX_EDIT_LINE_SKIP (gdxReduceFlash ? 0 : (D_800DCD04 != 0))
#else
...
Gfx* func_xk2_800E8F7C(Gfx* gfx) {
    ...
    if (D_80030060[0] == '\0') {
        ...
        /* "No Title" is a plain literal in this function on both the retail and the
           translated build, at retail's 0xE8/0xB2. Eight cells is 232..296, exactly the
           interior drawn above, so the final 'e' sitting flush against the border column
           is correct, not a clipping bug. */
