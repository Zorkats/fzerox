...
/* Array bound for Expansion Kit text that gdx_ek_strings_apply() rebinds from the loaded
 * 64DD disk. The English is longer than the Japanese it replaces (43 bytes against 20),
 * so an array still sized by its Japanese initializer is overrun. Must stay equal to
 * GDX_EK_TEXT_CAP in port/gen/EkTranslatedStrings.h. Empty outside the port so a plain
 * decomp build keeps the original layout. */
#ifdef PORT
