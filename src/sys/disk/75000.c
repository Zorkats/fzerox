...
/* No header declares these, so each call fell back to C's implicit-int rule. Harmless here, but
 * the same rule truncated a returned Gfx* to 32 bits in course_edit/1A2D70.c and crashed "Test
 * Course"; declaring them keeps /we4013 clean so that class cannot come back. They live in this
 * disk-only TU rather than a shared header because the superseded sys/rom twins differ in
 * parameter types (u32 vs s32 lba, size_t vs s32 sizes). */
...
#ifdef PORT
/* Transient disk prompts ("now saving" and friends) never appear on the port. Hardware spends
 * many frames on the physical drive with the prompt id latched in D_807C6EA8.unk_08, so the EK
 * renderer draws it; here the drive is synchronous and osSendMesg hands off straight into this
 * priority-30 worker, which completes the whole operation and re-zeroes the prompt before
 * control returns to the sender. Raised and cleared between two draws, so never rendered.
 * Genuine errors are unaffected because they latch.
 *
 * These record what was raised so ABC40.c's minimum-display hold can consume it on the next
 * draw. Deliberately NOT cleared by func_80767940(): that is the completion step of the
 * unk_00 == 1/5 paths and would erase the record again. The stale-record guard lives on the
    ...
    while (true) {
...
#ifdef PORT
        /* Snapshot point for the transient-prompt hold (see gGdxDiskPromptRaised above). It has
           to sit after the ring-buffer entries have `continue`d out and before any branch below
           clears the prompt. Id 10 is skipped: ABC40.c's draw switch has no case 10, so it is
           the "run silently" id and holding it would occupy the slot with nothing to show. */
...
#ifdef PORT
        /* func_xk1_8002E368 (AB150.c) advances the EK MFS management ops ONE step per call and,
         * short of GOOD/COMMAND_TERMINATED, expects to be invoked again by a later 64DD
         * interrupt. This port's disk backing is fully synchronous, so that interrupt never
         * comes: the outer loop falls back to a blocking osRecvMesg, D_80794E14/D_80794E18 stay
         * latched at 1 forever, and every reader gated on them (A2E90.c's kanji renderer,
         * Course Edit's input gates) goes blank permanently. Re-drive the state machine here
         * instead, bounded so an unrecoverable condition falls through to the busy-flag rescue.
         *
         * LEO_ERROR_DATA_PHASE_ERROR is excluded because it is not a transient: it is the
         * deliberate multi-frame hand-off to the player (unk_08 = 3 "insert writable media",
         * unk_10 = 4 resume token). Re-driving it collapses that conversation into 64 same-frame
         * retries against a medium the player has had no chance to change, and the fail-safe
         * below would then destroy the resume token with the prompt still on screen. Excluded,
         * retail state stands and the re-drive comes from where it does on hardware: the
         * per-frame unk_10 == 2/3/4 osSendMesg at the top of func_xk1_8002ED64 (ABC40.c). */
        if (temp_v0_3 != LEO_ERROR_GOOD && temp_v0_3 != LEO_ERROR_COMMAND_TERMINATED &&
            ...
            for (portRetries = 0; portRetries < 64; portRetries++) {
                ...
                /* Mirror the console switch above so func_xk1_8002E368's unk_10 == 4
                   (read-only-media resume) branch sees the state it expects next iteration. */
                switch (temp_v0_3) {
                    case LEO_ERROR_DIAGNOSTIC_FAILURE:
                        D_807C6EA8.unk_10 = 2;
                        break;
                ...
                if (temp_v0_3 == LEO_ERROR_DATA_PHASE_ERROR) {
                    /* Same exclusion as the entry condition, for an earlier error that retried
                       its way into the media-swap hand-off. Breaking out rather than running to
                       the loop bound also keeps the fail-safe below off the resume token. */
                    break;
                }
            }
            /* The DATA_PHASE_ERROR exclusion applies to the fail-safe too: force-clearing here
               is the destructive step the exclusion exists to prevent. */
            if (temp_v0_3 != LEO_ERROR_GOOD && temp_v0_3 != LEO_ERROR_DATA_PHASE_ERROR) {
                /* Fail-safe, not fail-stuck: clear the busy flags so the UI recovers even from
                   a condition the bounded retry could not resolve. */
