...
/* sDecalMenuItems below used to reference D_4001B00/D_4001D40/D_4001F80/
 * D_40021C0/D_4002400, whose create_machine_textures.yaml offsets collide with
 * the MARK emblem strip's US-cart rebased offsets (MARK's real content sits at
 * symbol_addr - 0x900), so the LINE menu rendered emblem art. The correct LINE
 * tiles already exist as separate, unused symbols at exactly (old offset -
 * 0x900). Re-pointing here rather than editing the yaml offsets of D_4001D40 etc.
 * avoids introducing new duplicate-offset collisions with those pre-existing
 * symbols; see tools/gen_asset_bindings.py's duplicate-offset lint. */
...
#ifdef PORT
/* Cart-resident "SAVE AS" label (create_machine_textures + 0xD80), referenced
 * only by the translated-disk registration-widget retarget below. */
 ...
 * differs from retail JP (disassembled from both disks' xk1 .data at
 * sMachineRegistrationMenuItems, 0x80031C5C): item[0], blank on JP, points at the
 * disk's "LOAD" art, and item[1] at the cart-resident "SAVE AS" label. This build
 * compiles the JP table, so apply the same retargets when the translated disk is
 * active. The gfx bridge resolves the aCreateMachineSaveAsTex stub to live
 * segment-4 bytes at draw time; the host array itself stays empty and must never
 * be read here.
 *
 * That label is I4 48x16 like every other label in the widget, so the shared
 * drop-item draw path below needs no special case -- despite
 * create_machine_textures.yaml labelling the whole family "I8 24x16". The yaml is
 * wrong: both readings consume the same 384 bytes, which is why it went
 * unnoticed, but only I4 48x16 decodes to legible text, and the cart's own draw
 * of the sibling aCreateMachineUseTex goes through
 * MachineCreate_DrawTextureBlockI4 at 48x16 (machine_create_draw.c:1010). */
